...
model_name = "openai/whisper-medium"  # Change to "small" or "medium" for faster results
print("even at this point it is working")
processor_1 = AutoProcessor.from_pretrained(model_name)
print("even at this point it is working")
model_1 = AutoModelForSpeechSeq2Seq.from_pretrained(model_name)
print("you know what even here woww")
technical_skills= [
...
def extract_audio(video_path, audio_path="extracted_audio.wav"):
    ...
    video.audio.write_audiofile(audio_path, codec="pcm_s16le")  # Ensure lossless quality
    print("audio being extracted")
    return audio_path


def split_audio(audio_path, chunk_duration=30):

    """Splits audio into smaller chunks for Whisper."""
    audio_data, sr = librosa.load(audio_path, sr=16000)  # Ensure 16kHz
    chunk_length = chunk_duration * sr  # Convert seconds to samples

    chunks = [audio_data[i : i + chunk_length] for i in range(0, len(audio_data), chunk_length)]
    return chunks, sr

def transcribe_chunks(audio_path):
    """Transcribes audio in chunks to prevent truncation."""
    chunks, sr = split_audio(audio_path)
    full_transcription = ""
    print(type(processor_1))
    for i, chunk in enumerate(chunks):
        input_features = processor_1(chunk, return_tensors="pt").input_features
        with torch.no_grad():
            predicted_ids = model_1.generate(input_features)
        
        text = processor_1.batch_decode(predicted_ids, skip_special_tokens=True)[0]
...
def write_text(text):
    ...
    c.drawString(200, height - 50, "Structured Resume Data")
    c.setFont("Helvetica", 10)  # Set font size for readability
...
def convert_to_resume(transcription):
    genai.configure(api_key="AIzaSyDke45OLWf0jMmdJZ4NJ63vpEtFkUG6XnY")
    model = genai.GenerativeModel("gemini-1.5-pro")
    response = model.generate_content(f"Extract Name, Email, Phone, Education, Experience, Skills, and Projects from: {transcription}. Return only JSON.")

    # Store raw response text in a variable
    ...
    #y_position = height - 100
    write_text(resume_data)
    print(f"Resume PDF saved as {pdf_filename}")
...
def process_video(video_path, output_text_path):
    ...
    print(audio_path)
    transcription = transcribe_chunks(audio_path)
...
def upload_videos():
    ...
    for video_file in uploaded_files:
        ...
        result=process_video(save_path, output_text_path)
    ...
    return jsonify({"message": "Videos processed successfully", "results": response_data,"data":result}), 200
